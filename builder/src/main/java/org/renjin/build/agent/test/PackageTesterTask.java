...
public class PackageTesterTask implements Callable<Void> {
  ...
  private void reportTestResult(String testName, boolean passed, long millis) throws IOException {
    ...
    try {
      ...
      Test test;
      List<Test> tests = em.createQuery("select t from Test t where t.name = :name and t.rPackage.id = :package", Test.class)
      ...
      List<TestResult> results = em.createQuery("select tr from TestResult tr where " +
        "tr.test = :test and " +
        "tr.renjinCommit.id = :commitId and " +
        "tr.packageVersion.id = :packageVersionId", TestResult.class)
