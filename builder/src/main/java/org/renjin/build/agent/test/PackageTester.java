...
public class PackageTester {
  ...
  public void run() throws Exception {
    ...
    try(PrintWriter resultsWriter = new PrintWriter(new File(baseDir, "tests.log"))) {
      File manDir = new File(baseDir, "man");
      if(manDir.listFiles() != null) {
        for(File file : manDir.listFiles()) {
          if(file.getName().endsWith(".Rd")) {
            executeFile(resultsWriter, file, ExamplesParser.parseExamples(file));
          }
        }
      }
    }
  ...
  @VisibleForTesting
  void executeFile(PrintWriter resultsWriter, File sourceFile, String source) {
    try {
      executeTestFile(resultsWriter, sourceFile, source);
    } catch (IOException e) {
  ...
  private void executeTestFile(PrintWriter resultsWriter, File sourceFile, String sourceText) throws IOException {
    ...
    try {
      ...
      resultsWriter.println("OK/" + stopwatch.elapsed(TimeUnit.MILLISECONDS));
      resultsWriter.flush();
