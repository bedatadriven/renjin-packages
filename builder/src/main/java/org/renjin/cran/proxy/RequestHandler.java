...
{
  public static final Logger log = Logger.getLogger(RequestHandler.class.getName());

  private Socket clientSocket;
  private Config config;

  public RequestHandler (Config config, Socket clientSocket)
  {
    this.config = config;
    ...
    try
    {
      ...
      do
      {
        ...
        if (downloadURL == null)
        {
          ...
          log.log (Level.SEVERE, "Found no URL to download in request:\n"+fullRequest.toString());
        }
        else
        {
  ...
  private void serveURL (String downloadURL) throws IOException
  {
    ...
    if (type == null)
    {
      log.log(Level.WARNING, "Unknown extension "+ext+". Using content type text/plain.");
      type = "text/plain";
    }
    println (type);
    println ();
